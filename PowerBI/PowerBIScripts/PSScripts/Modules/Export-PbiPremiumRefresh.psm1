...
function Export-PbiPremiumRefresh{
    param(
        [hashtable]$Config,
        [string]$SinkContainer
    )
    Write-Host "Exporting Premium Refresh Schedules to $SinkContainer..."
    
    $premiumRefresh = Invoke-PowerBIRestMethod -Method Get -Url "$($Config.ApiGatewayUri)/v1.0/myorg/admin/capacities/refreshables?`$expand=capacity,group"
